...
description: Handles terraform/, render.yaml, vercel.json, docker-compose.yml, .env.example, .github/, and .claude/ harness files (settings, agents, commands, hooks). Treats infra as code with extra caution — every change must be reviewed by cross-cutting-reviewer before commit.
...
- Edit: `terraform/`, `render.yaml`, `vercel.json`, `docker-compose.yml`, `.env.example`, `.github/`, `.claude/` (settings, agents, commands).
...
- `git add terraform/`, `git add .claude/`, `git add .github/`, `git add docker-compose.yml`, `git add render.yaml`, `git add vercel.json`, `git add .env.example`, `git add Makefile`, `git add scripts/`, `git add .pre-commit-config.yaml`, `git diff`, `git status`.
...
- Modifying `backend/alembic/versions/*` filenames (CLAUDE.md rule).
...
2. `yamllint -c .yamllint.yml render.yaml docker-compose.yml` if YAML changed
...
- A change affects production infrastructure (terraform apply implications, render.yaml service definition for a live service).
