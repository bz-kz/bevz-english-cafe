...
class ContactModel(Base, UUIDMixin, TimestampMixin):
    ...
    email: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="メールアドレス"
    )
    ...
    message: Mapped[str] = mapped_column(Text, nullable=False, comment="メッセージ")

    # レッスン関連
    lesson_type: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="レッスンタイプ"
    )

    preferred_contact: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="希望連絡方法"
    )

    # ステータス管理
    status: Mapped[str] = mapped_column(
        String(20), nullable=False, default="pending", comment="ステータス"
    )

    # 処理情報
    processed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="処理日時"
    )

    processed_by: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="処理者"
    )

    processing_notes: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="処理メモ"
    )
