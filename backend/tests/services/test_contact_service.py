...
class TestContactService:
    ...
    async def test_create_contact_success(
        self, contact_service, mock_repository, mock_email_service
    ):
        """問い合わせ作成成功テスト"""
        # モックの設定
        contact_id = uuid4()
        ...
        mock_email_service.send_contact_confirmation.assert_called_once_with(
            saved_contact
        )
    ...
    async def test_create_contact_email_failure(
        ...
        mock_repository.save.return_value = saved_contact
        # メール送信を失敗させる
        mock_email_service.send_contact_notification.side_effect = Exception(
            "Email failed"
        )
        mock_email_service.send_contact_confirmation.side_effect = Exception(
            "Email failed"
        )
    ...
    async def test_get_contact_by_id_success(self, contact_service, mock_repository):
        """ID指定での問い合わせ取得成功テスト"""
        contact_id = uuid4()
    ...
    async def test_get_contact_by_id_not_found(self, contact_service, mock_repository):
        """存在しないIDでの問い合わせ取得テスト"""
        contact_id = uuid4()
    ...
    async def test_update_contact_status_success(
        self, contact_service, mock_repository
    ):
        """問い合わせステータス更新成功テスト"""
        contact_id = uuid4()
    ...
    async def test_update_contact_status_not_found(
        ...
        result = await contact_service.update_contact_status(
            contact_id=contact_id, status=ContactStatus.COMPLETED
        )
