...
|---|---|---|
| `english-cafe-prod-wif` | CLI-driven | Remote |
| `english-cafe-prod-firestore` | CLI-driven | Remote |
| `english-cafe-prod-cloudrun` | CLI-driven | Remote |

For each: choose "CLI-driven workflow" so `terragrunt apply` can drive remote runs.
...
- `google_cloud_run_v2_service_iam_member.public` — `allUsers` invoker
- `google_cloud_run_domain_mapping.this[0]` — mapping for `api.bz-kz.com`
...
## 6. DNS for the custom domain

The `custom_domain_dns_records` output lists the records Cloud Run needs at the DNS provider. For an apex/subdomain like `api.bz-kz.com` you typically get a single `CNAME` to `ghs.googlehosted.com` or four `A`/`AAAA` records depending on the resource record type.

Add the records to whatever DNS provider hosts `bz-kz.com`. Then:

```bash
# Confirm DNS propagation
dig +short api.bz-kz.com

# Watch the domain mapping status
gcloud beta run domain-mappings describe --domain api.bz-kz.com --region asia-northeast1
```

Wait for `READY` and for Google-managed cert provisioning (up to ~15 min after DNS propagates).

Confirm:
```bash
curl https://api.bz-kz.com
# → "Congratulations | Cloud Run"
```
...
On `english-cafe-prod-cloudrun`, update the `image` Terraform Variable to the value printed above (e.g. `asia-northeast1-docker.pkg.dev/english-cafe-496209/english-cafe/api:228b23f`).
...
Note: the cloud-run-service module has `lifecycle.ignore_changes = [template[0].containers[0].image, ...]`. Terraform will detect a "change" on first apply with this new image but should mark it for in-place update of the service. Subsequent image bumps via CD will not require terraform applies.
