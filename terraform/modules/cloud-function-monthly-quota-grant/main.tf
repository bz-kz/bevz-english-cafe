...
resource "google_pubsub_topic" "monthly" {
  project    = var.gcp_project_id
  name       = "monthly-quota-grant-monthly"
...
resource "google_service_account" "fn" {
  ...
  display_name = "Monthly quota grant"
}
...
resource "google_storage_bucket" "source" {
  project                     = var.gcp_project_id
  name                        = "${var.gcp_project_id}-monthly-quota-grant-source"
  location                    = var.region
...
resource "google_cloudfunctions2_function" "monthly_quota_grant" {
  project  = var.gcp_project_id
  location = var.region
  name     = "monthly-quota-grant"

  build_config {
    runtime     = "python312"
    entry_point = "grant_monthly_quota"
    source {
      storage_source {
        bucket = google_storage_bucket.source.name
  ...
  event_trigger {
    ...
    pubsub_topic   = google_pubsub_topic.monthly.id
...
resource "google_cloud_scheduler_job" "monthly" {
  ...
  pubsub_target {
    topic_name = google_pubsub_topic.monthly.id
    data       = base64encode("{}")
  }
}
